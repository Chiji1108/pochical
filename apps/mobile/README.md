...
このドキュメントのパスとCLIコマンドは、特記がなければ `apps/mobile` を基準にしています。
依存関係のインストールはモノレポのルートで行います。

## iOSでのローカル開発

XcodeとiOSランタイム、Bun、Node.js（22.13以上の22系、24.3以上の24系、または26以上）が必要です。

1. モノレポのルートで `bun install` を実行し、その後 `cd apps/mobile` で移動します。
...
既存の開発環境では `bunx expo prebuild --platform ios --no-install`、`cd ios && pod install`、`apps/mobile` で `bun run ios` を順に実行してください。Metroの再起動やOTA更新だけでは反映されません。
