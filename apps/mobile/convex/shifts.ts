...
import { type ShiftRecord, shiftShape } from "../shared/work-schema";
    ...
    return await Promise.all(
      rows.map(async (row) => {
        const state = await ctx.runQuery(
          ...
          { collection: `shifts:${ownerId}`, document: row.id }
        );
...
export const materialize = internalMutation({
  args: { record: shiftShape, deleted: v.boolean() },
  handler: async (ctx, { record, deleted }) => {
    ...
    if (existing) {
      ...
      await ctx.db.insert("shifts", value);
    }
  },
});
...
export const replicate = mutation({
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    try {
      ...
      const record = doc.getMap("fields").toJSON() as ShiftRecord;
      ...
      await ctx.runMutation(internal.shifts.materialize, { record, deleted });
