...
Jazz 2 alpha で管理している個人シフトデータとローカルファースト identity を InstantDB に置き換える。置き換え後はアプリ内から `jazz-tools` / `jazz-rn` / `JazzProvider` / `useAll` / `useDb` / `useSession` を削除し、InstantDB の `init` / `db.useQuery` / `db.transact` / `db.useAuth` に統一する。
...
2026-05-22 時点で、Jazz の実装依存は撤去済み。

- `jazz-tools` / `jazz-rn` は `package.json` / `bun.lock` から削除済み
- `src/schema.ts` / `src/permissions.ts` は削除済み
- `metro.config.mjs` の `withJazz` は削除済み
- アプリ起動時は InstantDB Guest Auth で匿名ログインし、MMKV store を使う
- 個人シフトデータは `owner` link で `$users` に紐付ける
- `members` は InstantDB entity と UI 型の両方で `shiftMembers` に寄せた
- シフト表示/編集は `shift.pattern` / `shift.shiftMembers` / `pattern.nextDayPattern` の link shape を直接扱う
- グループ、招待、チャット、未読、presence は Convex のまま維持し、識別子名は `instantUserId` に変更済み
- 検証コマンド: `bun x tsc --noEmit`, `bun x ultracite check`
...
- `instant.schema.ts` / `instant.perms.ts` は作成済み
...
- Convex schema/functions and UI 引数名は `instantUserId` にリネーム済み
...
- Cloud dashboard に作成 data が反映される
