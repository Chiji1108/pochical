...
import { type PatternRecord, patternShape } from "../shared/work-schema";
    ...
    return await Promise.all(
      rows.map(async (row) => {
        const state = await ctx.runQuery(
          ...
          { collection: `shiftPatterns:${ownerId}`, document: row.id }
        );
...
export const materialize = internalMutation({
  args: { record: patternShape, deleted: v.boolean() },
  handler: async (ctx, { record, deleted }) => {
    ...
    if (existing) {
      ...
      await ctx.db.insert("shiftPatterns", value);
    }
  },
});
...
export const replicate = mutation({
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    const collection = `shiftPatterns:${ownerId}`;
    const doc = new YDoc();
    try {
      ...
      const record = doc.getMap("fields").toJSON() as PatternRecord;
      ...
      await ctx.runMutation(internal.shiftPatterns.materialize, {
        record,
        deleted,
      });
