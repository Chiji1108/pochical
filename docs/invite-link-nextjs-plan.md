...
EAS Hosting に招待 API はデプロイしない。Expo repo には招待 API Route を置かず、Next.js 側に `/pochical/api/invites` を実装する。
...
```txt
https://chiji.tech/pochical/invite/<inviteId>
```

Expo アプリ側は次を設定する。

```txt
EXPO_PUBLIC_INVITE_BASE_URL=https://chiji.tech/pochical
```
...
```txt
pochical:invite:<inviteId>
```

## Next.js 側で実装するもの

### `POST /pochical/api/invites`
...
- `INVITE_BASE_URL` または production host + `/pochical` から invite URL を組み立てる。
...
```json
{
  ...
  "url": "https://chiji.tech/pochical/invite/8Z4-yemWWxkV"
}
```

MVPでは Jazz backend で招待作成者の membership は検証しない。

### `GET /pochical/api/invites/[inviteId]`
...
- MVP では `pochical://invite/<inviteId>` の「アプリで開く」ボタンを置く。
