...
`authSubject` は Apple / Google 連携後に保存する。未連携ユーザーは `authSubject` を持たない。

MVP では Convex functions が args の `jazzUserId` を user identity として扱う。これは強い認証境界ではなく、身内利用と開発中の割り切りである。

将来 Apple / Google 連携を入れた後は、`ctx.auth.getUserIdentity()` から `authSubject` を取り、`users.authSubject -> jazzUserId` を引く。連携済みユーザーについては args の `jazzUserId` を信じない。
...
- `users.ensureAnonymous({ jazzUserId })`
  - Expo app 起動後、Jazz session が準備できたら呼ぶ。
  - `jazzUserId` の user row がなければ作成する。
  - 既にあれば `lastSeenAt` を更新する。
  - Apple / Google ログインは要求しない。

- `users.linkAccount({ jazzUserId })`
  - Apple / Google 連携後に呼ぶ。
  - `ctx.auth.getUserIdentity()` を必須にする。
  - `identity.tokenIdentifier` を `authSubject` として保存する。
  - 既存の `jazzUserId` と紐付ける。
  - 以後、機種変更や複数端末では `authSubject` から `jazzUserId` を復元する。

- `users.getLinkedUser()`
  - `ctx.auth.getUserIdentity()` から `authSubject` を取得する。
  - 紐付いた `jazzUserId` を返す。
  - バックアップ/復元 UI で使う。
...
- Apple / Google 連携を追加する。
- Jazz account recovery と Convex `users.linkAccount` を同じ UI から実行する。
- 連携済みユーザーは `authSubject -> jazzUserId` で復元できるようにする。
