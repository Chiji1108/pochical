...
```txt
chiji-tech/
  src/app/nurse-shift/page.tsx
  src/app/nurse-shift/invite/[inviteId]/page.tsx
  src/app/nurse-shift/api/invites/route.ts
  src/app/nurse-shift/api/invites/[inviteId]/route.ts
  src/lib/nurse-shift/invites.ts
```

現在の invite は `src/lib/nurse-shift/invites.ts` が Upstash Redis に `groupId`, `groupName`, `expiresAt` を保存する。`POST /nurse-shift/api/invites` が invite ID を発行し、`GET /nurse-shift/api/invites/[inviteId]` と landing page が同じ Redis record を読む。

Convex 移行後はこの Redis invite store と API Routes を削除する。landing route は `/nurse-shift/invite/[inviteCode]` に rename する。Next.js 側に Convex schema/functions や generated API 型は持ち込まない。
...
```txt
nurse-shift/src/app/(tabs)/group.tsx
...
nurse-shift/src/app/invite/[inviteId].tsx
  - GET .../api/invites/[inviteId] で preview する
  - app.shareGroupMembers に insert/update して参加する

nurse-shift/src/app/share-groups/[groupId].tsx
...
- `/nurse-shift/invite/[inviteCode]` の landing / fallback ページ
- OGP 表示
...
Next.js 側は `/nurse-shift/invite/[inviteCode]` の Universal Link / landing / fallback だけを担当する。landing page でグループ名を表示するため、Next.js server component 側から Convex の invite preview HTTP action を読む。
...
```txt
NURSE_SHIFT_CONVEX_HTTP_URL=https://<deployment>.convex.site
```
...
`GET /nurse-shift/invite/[inviteCode]` は Convex の invite preview を読み、以下を表示する。
...
現行 `chiji-tech/src/app/nurse-shift/invite/[inviteId]/page.tsx` は `params: Promise<{ inviteId: string }>` の Next.js 16 形式。移行時に `chiji-tech/src/app/nurse-shift/invite/[inviteCode]/page.tsx` へ rename し、`params: Promise<{ inviteCode: string }>` として扱う。`getStoredInvite` import は Convex HTTP fetch helper に置き換える。

### Next.js の具体作業

- `chiji-tech/src/lib/nurse-shift/invites.ts` を Redis store から Convex HTTP preview fetch helper に置き換える、または `invite-preview.ts` のような読み取り専用 helper に分ける。
- `chiji-tech/src/app/nurse-shift/invite/[inviteId]/page.tsx` を `chiji-tech/src/app/nurse-shift/invite/[inviteCode]/page.tsx` に rename し、Convex preview helper を使う。
- `chiji-tech/src/app/nurse-shift/api/invites/route.ts` を削除する。
- `chiji-tech/src/app/nurse-shift/api/invites/[inviteId]/route.ts` を削除する。
- `chiji-tech/package.json` から `@upstash/redis` を削除する。
- `chiji-tech/src/app/nurse-shift/page.tsx` の `TECH_BADGES` から `Upstash Redis` を外し、Convex に差し替える。
- `chiji-tech/src/app/nurse-shift/page.tsx` のプライバシーポリシー文言を「招待リンクの発行と確認は Convex に保存する招待情報を使う」方針に更新する。署名付きトークンや期限付き招待前提の表現は削除する。
...
```txt
INVITE_BASE_URL=https://chiji.tech/nurse-shift
```
...
- Convex env に `INVITE_BASE_URL=https://chiji.tech/nurse-shift` を設定する。
...
- `chiji-tech` に `NURSE_SHIFT_CONVEX_HTTP_URL` を設定する。
- `chiji-tech` に Convex HTTP preview fetch helper を追加する。
- `chiji-tech/src/app/nurse-shift/invite/[inviteId]/page.tsx` を `chiji-tech/src/app/nurse-shift/invite/[inviteCode]/page.tsx` に rename し、Convex HTTP preview に切り替える。
...
- `chiji-tech/src/app/nurse-shift/page.tsx` の技術バッジとプライバシー文言を Convex 前提に更新する。
- `chiji-tech/src/app/nurse-shift/api/invites/route.ts` を削除する。
- `chiji-tech/src/app/nurse-shift/api/invites/[inviteId]/route.ts` を削除する。
- `chiji-tech/src/lib/nurse-shift/invites.ts` の Redis 書き込み処理を削除する。preview helper を別ファイルにした場合はファイルごと削除する。
...
- `chiji-tech/src/app/nurse-shift/page.tsx` の技術バッジ/プライバシー文言が Convex 前提になっている。
