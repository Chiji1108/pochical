...
Clients open one WebSocket per group at `/v1/groups/{groupId}/socket`. Every message is binary: clients send `pochical.v1.ClientFrame`, the server sends `pochical.v1.ServerFrame` (see `proto/pochical/v1/sync.proto`). Text messages are protocol errors.
...
2. The first frame must be `Hello { protocol_version, cursor }`.
   ...
   - Otherwise it replies `Welcome { cursor }` with the head of the group's change log.
